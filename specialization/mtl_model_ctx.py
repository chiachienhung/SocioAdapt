...
class MultiTaskModel(nn.Module):
    def __init__(self, encoder_name_or_path, cache_dir, mask_ids, tasks: List):
        super().__init__()
    ...
    def _create_output_head(self, encoder_hidden_size: int, task):
        ...
        elif task.type == "mask_seq_classification":
            return MLMSeqHead(encoder_hidden_size, task.num_labels, self.mask_ids)
        else:
        ...
        **kwargs,
        ...
        for unique_task_id in unique_task_ids_list:
            ...
            logits, task_loss = self.output_heads[str(unique_task_id)].forward(
                ...
                input_ids = input_ids[task_id_filter]
            )
...
class MLMSeqHead(nn.Module):
    def __init__(self, hidden_size, num_labels, mask_ids, dropout_p=0.1):
        super().__init__()
        self.num_labels = num_labels
        self.mask_ids = mask_ids
        self.dropout = nn.Dropout(dropout_p)
    ...
    def forward(self, sequence_output, pooled_output, input_ids, labels=None, **kwargs):
        pooled_output = self.dropout(pooled_output)
        pred_masks = (input_ids == self.mask_ids)
        result = []
        for i in range(len(sequence_output)):
            ctx_mask = sequence_output[i][pred_masks[i]]
            if ctx_mask.sum(0).sum(0)==0:
                result.append(pooled_output[i])
            else:
                result.append(ctx_mask.sum(0)/len(ctx_mask))
        final = torch.stack(result)
