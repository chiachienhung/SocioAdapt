...
from mtl_model_cls import MultiTaskModel, MultiTaskModelWeight
import datasets
...
def tokenize_MLM_dataset(
    ...
    if data_args.line_by_line:
        ...
        def tokenize_function(examples):
            ...
            #result["task_ids"] = [task_id]*len(raw_datasets["train"])
...
def load_seq_classification_dataset(task_id, tokenizer, data_args, training_args, model_args):
    ...
    #raw_datasets = raw_datasets.map(lambda example: {'label': example['text'].split('\t')[0]})
    raw_datasets = raw_datasets.map(lambda example: {'text': example['text'].split('\t')[1]})
    ...
    task_info = Task(
        id=task_id, name="seqclass", num_labels=2, type="seq_classification"
    )
...
class DataCollatorForMTL:
    ...
    def collate_batch(self, features: List[Union[InputDataClass, Dict]]) -> Dict[str, torch.Tensor]:
        ...
        index = torch.nonzero(textclass_result['task_ids'] == 0).view(-1).detach().numpy()
...
def main():
    ...
    print(tasks)
    #model = MultiTaskModel(model_args.model_name_or_path, model_args.cache_dir, tasks)
    model = MultiTaskModelWeight(model_args.model_name_or_path, model_args.cache_dir, tasks)
